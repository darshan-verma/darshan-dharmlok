...
import type { ProkeralaUpagrahaPositionResponse } from "@/types/prokerala";
import {
...
export async function GET(request: NextRequest) {
	...
	const laValidation = validateLanguage<UpagrahaLanguageCode>(
		laRaw,
		DAILY_PANCHANG_LANGUAGE_CODES
	);
	if (!laValidation.ok) {
	...
	const query: Record<string, string> = {
		...
		coordinates,
		datetime,
		upagraha_position: upagrahaValidation.value ?? "middle",
	};
	...
	try {
		const response = await prokeralaFetch<ProkeralaUpagrahaPositionResponse>(
			"astrology/upagraha-position",
			{ query }
		);
		return NextResponse.json(response);
	} catch (error) {
		const message = error instanceof Error ? error.message : "Unknown error";
		console.error("Prokerala upagraha-position API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Upagraha position request failed",
				details: message,
			},
			{ status: 500 }
		);
