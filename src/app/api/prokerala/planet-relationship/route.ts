...
import type { ProkeralaPlanetRelationshipResponse } from "@/types/prokerala";
import {
	isValidCoordinates,
	isValidDateTime,
	normalizeDateTime,
...
type PlanetRelationshipLanguageCode =
...
export async function GET(request: NextRequest) {
	...
	const laValidation = validateLanguage<PlanetRelationshipLanguageCode>(
		laRaw,
		PLANET_POSITION_LANGUAGE_CODES
	);
	if (!laValidation.ok) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaPlanetRelationshipResponse>(
			"astrology/planet-relationship",
			{ query }
		);
		return NextResponse.json(response);
	} catch (error) {
		const message = error instanceof Error ? error.message : "Unknown error";
		console.error("Prokerala planet-relationship API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Planet relationship request failed",
				details: message,
			},
			{ status: 500 }
		);
