...
import type { ProkeralaDashaPeriodsResponse } from "@/types/prokerala";
import {
	isValidCoordinates,
	isValidDateTime,
	normalizeDateTime,
...
const DASHA_PERIOD_LANGUAGE_CODES = ["en", "ta", "ml", "hi"] as const;
type DashaPeriodLanguageCode = (typeof DASHA_PERIOD_LANGUAGE_CODES)[number];
...
export async function GET(request: NextRequest) {
	...
	const laValidation = validateLanguage<DashaPeriodLanguageCode>(
		laRaw,
		DASHA_PERIOD_LANGUAGE_CODES
	);
	if (!laValidation.ok) {
		return NextResponse.json(
	...
	if (
		yearLengthRaw != null &&
		yearLengthRaw !== "" &&
		!YEAR_LENGTH_VALUES.includes(
			yearLengthRaw as (typeof YEAR_LENGTH_VALUES)[number]
		)
	) {
		return NextResponse.json(
			{
	...
	try {
		const response = await prokeralaFetch<ProkeralaDashaPeriodsResponse>(
		...
		console.error("Prokerala dasha-periods API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Dasha periods request failed",
				details: message,
			},
			{ status: 500 }
		);
