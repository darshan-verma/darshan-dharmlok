...
import type { ProkeralaDishaShoolResponse } from "@/types/prokerala";
import {
...
export async function GET(request: NextRequest) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaDishaShoolResponse>(
			"astrology/disha-shool",
			{ query }
		);
		return NextResponse.json(response);
	} catch (error) {
		const message = error instanceof Error ? error.message : "Unknown error";
		console.error("Prokerala disha-shool API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Disha Shool request failed",
				details: message,
			},
			{ status: 500 }
		);
