...
import type { ProkeralaHoraTimingResponse } from "@/types/prokerala";
import {
...
export async function GET(request: NextRequest) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaHoraTimingResponse>(
			"astrology/hora",
			{ query }
		);
		...
		console.error("Prokerala hora API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Hora request failed",
				details: message,
			},
			{ status: 500 }
		);
