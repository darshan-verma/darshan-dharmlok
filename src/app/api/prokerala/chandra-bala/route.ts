...
import type { ProkeralaChandraBalaResponse } from "@/types/prokerala";
import {
...
export async function GET(request: NextRequest) {
	...
	// language (optional, per Daily Panchang docs: en|hi|ta|te|ml)
	...
	try {
		const response = await prokeralaFetch<ProkeralaChandraBalaResponse>(
			"astrology/chandra-bala",
			{ query }
		);
		return NextResponse.json(response);
	} catch (error) {
		const message = error instanceof Error ? error.message : "Unknown error";
		console.error("Prokerala chandra-bala API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Chandra Bala request failed",
				details: message,
			},
			{ status: 500 }
		);
