...
import type { ProkeralaMangalDoshaResponse } from "@/types/prokerala";
import {
	isValidCoordinates,
	isValidDateTime,
	normalizeDateTime,
...
export async function GET(request: NextRequest) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaMangalDoshaResponse>(
		...
		console.error("Prokerala mangal-dosha API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Mangal dosha request failed",
				details: message,
			},
			{ status: 500 }
		);
