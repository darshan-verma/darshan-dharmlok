...
import type { ProkeralaYogaResponse } from "@/types/prokerala";
import {
	isValidCoordinates,
	isValidDateTime,
	normalizeDateTime,
...
type YogaLanguageCode = (typeof YOGA_LANGUAGE_CODES)[number];
...
export async function GET(request: NextRequest) {
	...
	const laValidation = validateLanguage<YogaLanguageCode>(
		laRaw,
		YOGA_LANGUAGE_CODES
	);
	if (!laValidation.ok) {
	...
	if (planetRaw != null && planetRaw.trim() !== "") {
		query.planet = planetRaw.trim();
	}

	try {
		const response = await prokeralaFetch<ProkeralaYogaResponse>(
			"astrology/yoga",
			{ query }
		);
		...
		console.error("Prokerala yoga API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Yoga request failed",
				details: message,
			},
			{ status: 500 }
		);
