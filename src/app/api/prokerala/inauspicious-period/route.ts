...
import {
	type ProkeralaInauspiciousPeriodResponse,
...
export async function GET(request: NextRequest) {
	...
	// language (optional, restricted set for these endpoints)
	const languageResult = validateLanguage(
		laRaw,
		DAILY_PANCHANG_LANGUAGE_CODES
	);
	...
	try {
		const response =
			await prokeralaFetch<ProkeralaInauspiciousPeriodResponse>(
		...
		console.error("Prokerala inauspicious-period API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Inauspicious period request failed",
				details: message,
			},
			{ status: 500 }
		);
