...
import type { ProkeralaTaraBalaResponse } from "@/types/prokerala";
import {
...
export async function GET(request: NextRequest) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaTaraBalaResponse>(
			"astrology/tara-bala",
			{ query }
		);
		...
		console.error("Prokerala tara-bala API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Tara Bala request failed",
				details: message,
			},
			{ status: 500 }
		);
