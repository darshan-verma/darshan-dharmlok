...
import type { ProkeralaSolsticeResponse } from "@/types/prokerala";
import {
...
export async function GET(request: NextRequest) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaSolsticeResponse>(
			"astrology/solstice",
			{ query }
		);
		...
		console.error("Prokerala solstice API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Solstice request failed",
				details: message,
			},
			{ status: 500 }
		);
