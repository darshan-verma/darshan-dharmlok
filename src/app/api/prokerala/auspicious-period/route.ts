...
import {
	type ProkeralaAuspiciousPeriodResponse,
...
export async function GET(request: NextRequest) {
	...
	try {
		const response =
			await prokeralaFetch<ProkeralaAuspiciousPeriodResponse>(
		...
		console.error("Prokerala auspicious-period API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Auspicious period request failed",
				details: message,
			},
			{ status: 500 }
		);
