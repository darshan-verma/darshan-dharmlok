...
import type { ProkeralaRituResponse } from "@/types/prokerala";
import {
...
export async function GET(request: NextRequest) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaRituResponse>("astrology/ritu", {
			query,
		});
		return NextResponse.json(response);
	} catch (error) {
		const message = error instanceof Error ? error.message : "Unknown error";
		console.error("Prokerala ritu API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Ritu request failed",
				details: message,
			},
			{ status: 500 }
		);
