...
import type { ProkeralaGowriNallaNeramResponse } from "@/types/prokerala";
import {
...
export async function GET(request: NextRequest) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaGowriNallaNeramResponse>(
		...
		console.error("Prokerala gowri-nalla-neram API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Gowri Nalla Neram request failed",
				details: message,
			},
			{ status: 500 }
		);
