...
export async function GET(request: NextRequest) {
	...
	try {
		const svg = await prokeralaFetchChart("astrology/sarvashtakavarga-chart", {
			query,
		});
		...
		console.error("Prokerala sarvashtakavarga-chart API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Sarvashtakavarga chart request failed",
				details: message,
			},
			{ status: 500 }
		);
