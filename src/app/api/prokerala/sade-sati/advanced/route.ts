...
import type { ProkeralaSadeSatiAdvancedResponse } from "@/types/prokerala";
import {
	isValidCoordinates,
	isValidDateTime,
	normalizeDateTime,
...
export async function GET(request: NextRequest) {
	...
	try {
		const response =
			await prokeralaFetch<ProkeralaSadeSatiAdvancedResponse>(
				"astrology/sade-sati/advanced",
				{ query }
			);
		return NextResponse.json(response);
	} catch (error) {
		const message = error instanceof Error ? error.message : "Unknown error";
		console.error("Prokerala sade-sati/advanced API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Detailed sade sati request failed",
				details: message,
			},
			{ status: 500 }
		);
