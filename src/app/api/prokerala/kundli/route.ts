...
import type { ProkeralaKundliResponse } from "@/types/prokerala";
import {
	isValidCoordinates,
	isValidDateTime,
	normalizeDateTime,
...
export async function GET(request: NextRequest) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaKundliResponse>(
			"astrology/kundli",
			{ query }
		);
		...
		console.error("Prokerala kundli API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Kundli request failed",
				details: message,
			},
			{ status: 500 }
		);
