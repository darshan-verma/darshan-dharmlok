...
import type { ProkeralaPoruthamAdvancedResponse } from "@/types/prokerala";
import {
...
export async function GET(request: NextRequest) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaPoruthamAdvancedResponse>(
			"astrology/porutham/advanced",
			{ query }
		);
		return NextResponse.json(response);
	} catch (error) {
		const message = error instanceof Error ? error.message : "Unknown error";
		console.error("Prokerala porutham/advanced API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Detailed porutham request failed",
				details: message,
			},
			{ status: 500 }
		);
