...
import type { ProkeralaChoghadiyaResponse } from "@/types/prokerala";
import {
	isValidCoordinates,
	isValidDateTime,
	normalizeDateTime,
...
const CHOGHADIYA_LANGUAGE_CODES = ["en", "hi"] as const;
...
export async function GET(request: NextRequest) {
	...
	// language (optional, per choghadiya docs: en|hi)
	const languageResult = validateLanguage(laRaw, CHOGHADIYA_LANGUAGE_CODES);
	...
	try {
		const response = await prokeralaFetch<ProkeralaChoghadiyaResponse>(
		...
		console.error("Prokerala choghadiya API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Choghadiya request failed",
				details: message,
			},
			{ status: 500 }
		);
