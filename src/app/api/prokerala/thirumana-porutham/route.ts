...
import type { ProkeralaThirumanaPoruthamResponse } from "@/types/prokerala";
import {
	validateNakshatraId,
...
export async function GET(request: NextRequest) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaThirumanaPoruthamResponse>(
			"astrology/thirumana-porutham",
			{ query }
		);
		return NextResponse.json(response);
	} catch (error) {
		const message = error instanceof Error ? error.message : "Unknown error";
		console.error("Prokerala thirumana-porutham API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Thirumana porutham request failed",
				details: message,
			},
			{ status: 500 }
		);
