...
import type { ProkeralaKundliMatchingResponse } from "@/types/prokerala";
import {
...
export async function GET(request: NextRequest) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaKundliMatchingResponse>(
		...
		console.error("Prokerala kundli-matching API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Kundli matching request failed",
				details: message,
			},
			{ status: 500 }
		);
