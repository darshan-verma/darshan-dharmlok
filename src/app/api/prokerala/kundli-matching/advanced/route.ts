...
import type { ProkeralaKundliMatchingAdvancedResponse } from "@/types/prokerala";
import {
	KUNDLI_MATCHING_LANGUAGE_CODES,
	validateAyanamsa,
...
type KundliMatchingLanguageCode =
	(typeof KUNDLI_MATCHING_LANGUAGE_CODES)[number];
...
export async function GET(request: NextRequest) {
	...
	const laValidation = validateLanguage<KundliMatchingLanguageCode>(
		laRaw,
		KUNDLI_MATCHING_LANGUAGE_CODES
	);
	if (!laValidation.ok) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaKundliMatchingAdvancedResponse>(
		...
		console.error("Prokerala kundli-matching/advanced API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Detailed kundli matching request failed",
				details: message,
			},
			{ status: 500 }
		);
