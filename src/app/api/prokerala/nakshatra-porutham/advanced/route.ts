...
import type { ProkeralaNakshatraPoruthamAdvancedResponse } from "@/types/prokerala";
import {
	validateNakshatraId,
...
export async function GET(request: NextRequest) {
	...
	try {
		const response = await prokeralaFetch<ProkeralaNakshatraPoruthamAdvancedResponse>(
			"astrology/nakshatra-porutham/advanced",
			{ query }
		);
		return NextResponse.json(response);
	} catch (error) {
		const message = error instanceof Error ? error.message : "Unknown error";
		console.error("Prokerala nakshatra-porutham/advanced API error:", error);
		return NextResponse.json(
			{
				ok: false,
				error: "Detailed nakshatra porutham request failed",
				details: message,
			},
			{ status: 500 }
		);
