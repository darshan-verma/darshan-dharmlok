...
interface HotelDharamshalaApiResponse {
	id: string;
	name: string;
	phone: string;
	email: string;
...
export default function HotelDharamshalaPage() {
	...
	useEffect(() => {
		const fetchHotelDharamshalas = async () => {
			setLoading(true);
			try {
				// Use the new optimized hotel_dharamshala_vendor-specific API endpoint
				const response = await fetch(
					`/api/users/hotel_dharamshala_vendor?page=${pagination.currentPage}&limit=${pagination.itemsPerPage}`
				);
				...
				// Transform the optimized API response to match HotelDharamshala structure
				const mappedHotelDharamshalas = data.data.map(
	...
	const handleDelete = async () => {
		...
		try {
			// Use the new hotel_dharamshala_vendor-specific API endpoint for deletion
			const response = await fetch(
				`/api/users/hotel_dharamshala_vendor/${HotelDharamshalaToDelete.id}`,
				{
					method: "DELETE",
				}
			);
			...
			toast.success("HotelDharamshala deleted successfully");
		} catch (error) {
			console.error("Error deleting HotelDharamshala:", error);
			toast.error(
				error instanceof Error
					? error.message
					: "Failed to delete HotelDharamshala"
			);
		} finally {
