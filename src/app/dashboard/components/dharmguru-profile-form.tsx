...
export default function DharmguruProfileForm({
	...
	useEffect(() => {
		async function fetchProfile() {
			try {
				...
				const res = await fetch(`/api/users/dharmguru/${profile?.id}`);
				if (res.ok) {
					const data: DharmguruApiResponse = await res.json();
					setForm({
						name: data.name || "",
						email: data.email || "",
						phone: data.phone || "",
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const response = await fetch(`/api/users/dharmguru/${profile?.id}`, {
				method: "PUT",
			...
			if (profile?.id) {
				// Fetch updated profile using the new dharmguru-specific endpoint
				const res = await fetch(`/api/users/dharmguru/${profile.id}`);
				if (res.ok) {
					const data: DharmguruApiResponse = await res.json();
					const updatedForm = {
