  ...
  useEffect(() => {
    const fetchDharmgurus = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/users/dharmguru?${params.toString()}`);
        if (!response.ok) {
      ...
      <section className="py-16 bg-[#f5f5f0]">
        <div className="container mx-auto px-4 max-w-7xl">
          <SimpleSearchFilterBar
            ...
            searchPlaceholder="Search dharmgurus by name"
            religiousValue={selectedReligiousCategory}
            onReligiousChange={handleReligiousCategoryChange}
            onClear={clearFilters}
            resultText={
              pagination
                ? `${pagination.totalCount} dharmguru${pagination.totalCount !== 1 ? "s" : ""} found`
                : undefined
            }
          />
          {loading ? (
